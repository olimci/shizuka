...
var StepStatic = StepFunc(IDStatic, func(_ context.Context, sc StepContext) error {
	cfg := keys.GetAs(sc, keys.Config)
	sourceFS, _ := sc.Source()
	...
	files, err := fileutils.WalkFilesFS(sourceFS, sourceRoot)
	if err != nil {
		return err
	}
	...
	for _, rel := range files.Values() {
		...
		sc.Emit(manifest.StaticArtefact(manifest.Claim{
			Owner:  "static",
			Source: source,
			Target: target,
	...
	return nil
}).WithReads(string(keys.Config))

// StepPagesBuild creates the manifest artefacts for pages.
var StepPagesBuild = StepFunc(IDPagesBuild, func(_ context.Context, sc StepContext) error {
	opts := keys.GetAs(sc, keys.Options)
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
	site := keys.GetAs(sc, keys.Site)
	tmpl := keys.GetAs(sc, keys.Templates)
	...
	for _, node := range pages.Nodes() {
		...
		if node.Error != nil {
			if errTmpl := lookupErrPage(opts.PageErrTemplates, node.Error); errTmpl != nil {
				sc.Emit(manifest.TemplateArtefact(claim, errTmpl, transforms.PageTemplate{
					Site:  *site,
					Error: node.Error,
				}).Post(m))
			}
		...
		sc.Emit(manifest.NamedTemplateArtefact(claim, page.Meta.Template, tmpl, transforms.PageTemplate{
			Page: *page,
			Site: *site,
		}).Post(m))
	}
	...
	WithReads(
		...
		string(keys.Pages),
		string(keys.Site),
		string(keys.Templates),
	)

// StepPagesResolve creates the manifest registry entries for site information.
var StepPagesResolve = StepFunc(IDPagesResolve, func(_ context.Context, sc StepContext) error {
	opts := keys.GetAs(sc, keys.Options)
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
	buildCtx := keys.GetAs(sc, keys.BuildMeta)
	...
	keys.SetAs(sc, keys.Site, site)

	return nil
}).WithDeps(IDPagesRender).
	WithReads(
		string(keys.Options),
		string(keys.Config),
		string(keys.BuildMeta),
	).
	WithWrites(string(keys.Pages), string(keys.Site))

// StepPagesTemplates parses the page templates.
var StepPagesTemplates = StepFunc(IDPagesTemplates, func(_ context.Context, sc StepContext) error {
	config := keys.GetAs(sc, keys.Config)
	...
	sourceFS, _ := sc.Source()
	tmpl, err := parseTemplates(sourceFS, glob, transforms.DefaultTemplateFuncs())
	if err != nil {
		return fmt.Errorf("failed to parse templates: %w", err)
	}

	keys.SetAs(sc, keys.Templates, tmpl)

	return nil
}).WithReads(string(keys.Config)).WithWrites(string(keys.Templates))

// StepPagesRender renders page bodies from content.
var StepPagesRender = StepFunc(IDPagesRender, func(_ context.Context, sc StepContext) error {
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
...
var StepPagesIndex = StepFunc(IDPagesIndex, func(_ context.Context, sc StepContext) error {
	...
	sourceFS, _ := sc.Source()
	tree, err := fileutils.WalkTreeFS(sourceFS, root)
	if err != nil {
	...
	tree.Traverse(func(node *fileutils.FSNode, depth int) {
		...
		if ext == ".html" {
			...
			sc.Emit(manifest.StaticArtefact(manifest.NewPageClaim(source, url)))
			return
		}

		page, err := transforms.BuildPage(sourceFS, source)
		if err != nil {
	...
	keys.SetAs(sc, keys.Pages, pageTree)

	return nil
}).WithReads(string(keys.Config)).WithWrites(string(keys.Pages))

// StepHeaders writes a headers file from config.
var StepHeaders = StepFunc(IDHeaders, func(_ context.Context, sc StepContext) error {
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
	...
	headers := maps.Clone(headersCfg.Headers)
	...
	return nil
}).WithDeps(IDPagesResolve).
	WithReads(string(keys.Config), string(keys.Pages))

// StepRedirects writes a redirects file from config.
var StepRedirects = StepFunc(IDRedirects, func(_ context.Context, sc StepContext) error {
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
...
var StepRSS = StepFunc(IDRSS, func(_ context.Context, sc StepContext) error {
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
	site := keys.GetAs(sc, keys.Site)
	...
	sc.Emit(manifest.TemplateArtefact(
	...
	WithReads(string(keys.Config), string(keys.Pages), string(keys.Site))

var StepSitemap = StepFunc(IDSitemap, func(_ context.Context, sc StepContext) error {
	cfg := keys.GetAs(sc, keys.Config)
	pages := keys.GetAs(sc, keys.Pages)
	site := keys.GetAs(sc, keys.Site)
