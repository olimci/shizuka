...
func OpenDefault(opts Options) (*Console, error) {
	return Open(os.Stdin, os.Stdout, os.Stderr, opts)
}
...
func Open(in, out, errFile *os.File, opts Options) (*Console, error) {
	...
	if opts.NoEcho && c.InIsTerminal && supportsEchoControl() {
...
func (c *Console) ClearScreen() error {
	if c == nil || !c.OutIsTerminal {
		return nil
	}

	_, err := fmt.Fprint(c.Out, clearScreen, moveHome)
	return err
}

func (c *Console) ClearScrollback() error {
	if c == nil || !c.OutIsTerminal {
		return nil
	}

	_, err := fmt.Fprint(c.Out, clearScrollback)
	return err
}
